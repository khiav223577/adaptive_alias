...
class UserModelQueryingTest < Minitest::Test
  ...
  def test_group_and_count
    assert_queries([
      'SELECT COUNT(*) AS count_all, `users`.`profile_id` AS users_profile_id FROM `users` WHERE `users`.`id` = 1 GROUP BY `users`.`profile_id`',
    ]) do
    ...
    3.times do
      ...
      assert_queries([
        'SELECT COUNT(*) AS count_all, `users`.`profile_id` AS users_profile_id FROM `users` WHERE `users`.`id` = 1 GROUP BY `users`.`profile_id`',
        'SELECT COUNT(*) AS count_all, `users`.`profile_id_new` AS users_profile_id_new FROM `users` WHERE `users`.`id` = 1 GROUP BY `users`.`profile_id_new`',
      ]) do
        assert_equal({ 1 => 1 }, User.where(id: 1).group(:profile_id_new).count)
      end
